...
export async function POST(req: Request) {
  try {
    ...
    if (cloudinaryConfigured) {
      try {
        ...
        return NextResponse.json({ 
          success: true, 
          message: 'Favicon uploaded successfully to Cloudinary', 
          url: result.secure_url 
        }, { status: 200 });
      } catch (cloudinaryError: any) {
        console.error('Cloudinary favicon upload failure, falling back to local:', cloudinaryError);
      }
    }
    ...
    const filename = `favicon_${Date.now()}-${file.name.replace(/\s+/g, '_')}`;
    ...
    return NextResponse.json({ 
      success: true, 
      message: 'Favicon uploaded successfully locally', 
      url: fileUrl 
    }, { status: 200 });

  } catch (error: any) {
    console.error('Favicon upload endpoint failure:', error);
