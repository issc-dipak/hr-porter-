    ...
    cloudinary.uploader.upload_stream(
      {
        ...
        public_id: `login_banner_${Date.now()}`,
        resource_type: 'image'
      },
      (error, result) => {
...
export async function POST(req: Request) {
  try {
    ...
    if (cloudinaryConfigured) {
      try {
        ...
        return NextResponse.json({ 
          success: true, 
          message: 'Login banner uploaded successfully to Cloudinary', 
          url: result.secure_url 
        }, { status: 200 });
      } catch (cloudinaryError: any) {
        console.error('Cloudinary login banner upload failure, falling back to local:', cloudinaryError);
      }
    }
    ...
    const filename = `banner_${Date.now()}-${file.name.replace(/\s+/g, '_')}`;
    ...
    return NextResponse.json({ 
      success: true, 
      message: 'Login banner uploaded successfully locally', 
      url: fileUrl 
    }, { status: 200 });

  } catch (error: any) {
    console.error('Login banner upload endpoint failure:', error);
